...
# Load bootEnv.txt with corruption detection and .dist fallback.
...
setenv rootdev
if load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}bootEnv.txt; then
	env import -t ${load_addr} ${filesize}
fi
if test -z "${rootdev}"; then
	if load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}bootEnv.txt.dist; then
		echo "WARNING: bootEnv.txt corrupt, loading .dist fallback"
		setenv rootdev
		env import -t ${load_addr} ${filesize}
	fi
fi
...
# fdtfile is set by bootEnv.txt (per-board BOOT_FDT_FILE).
