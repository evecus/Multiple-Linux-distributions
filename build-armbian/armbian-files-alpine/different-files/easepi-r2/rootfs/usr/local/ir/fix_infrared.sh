#!/bin/bash
# EasePi-R2 红外初始化脚本 - Alpine/OpenRC variant
...
# 检查lircd服务状态（OpenRC，guarded for missing lircd）
if command -v rc-service >/dev/null 2>&1 && [ -x /etc/init.d/lircd ]; then
    if rc-service lircd status 2>/dev/null | grep -q "started"; then
        echo "[IR] lircd服务已经在运行"
    else
        echo "[IR] 启动lircd服务..."
        rc-service lircd restart 2>/dev/null || true
    fi
    rc-update add lircd default 2>/dev/null || true
fi
