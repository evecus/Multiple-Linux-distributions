#!/bin/bash
# EasePi-A2 蓝牙初始化脚本 (v2 - with GPIO reset) - Alpine/OpenRC variant
...
# Ensure bluetooth service is started (OpenRC, guarded for missing bluez)
if command -v rc-service >/dev/null 2>&1 && [ -x /etc/init.d/bluetooth ]; then
