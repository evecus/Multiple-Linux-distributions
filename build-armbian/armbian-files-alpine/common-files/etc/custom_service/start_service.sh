...
# Dependent script: /etc/local.d/start_service.start (OpenRC local service)
# File path: /etc/custom_service/start_service.sh
#
# Alpine (musl + OpenRC) variant: no dpkg / systemctl / armbian-tf references.
# Version: v1.0-alpine
#
#========================================================================================
...
# Restart ssh service (OpenRC init script is /etc/init.d/sshd on Alpine)
...
# Note: armbian-openvfd is not shipped in the Alpine tree (glibc-agnostic script dropped).
...
# Maximize root partition size (OpenRC variant: resize2fs directly, armbian-tf is not shipped)
