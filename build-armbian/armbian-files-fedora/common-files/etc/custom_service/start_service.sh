...
# Dependent script: /etc/systemd/system/custom-start.service (created by rebuild)
# File path: /etc/custom_service/start_service.sh
#
# Fedora (glibc + systemd) variant: no dpkg / apt-get / armbian-tf references.
# Version: v1.0-fedora
#
#========================================================================================
...
# 2) /boot/bootEnv.txt : FDT=/dtb/.../xxx.dtb  (or FDT=xxx.dtb) / fdtfile=vendor/xxx.dtb
[[ -z "${FDTFILE}" && -f "/boot/bootEnv.txt" ]] &&
    FDTFILE="$(grep -E '^(FDT|fdtfile)=.*\.dtb$' /boot/bootEnv.txt 2>/dev/null | head -n1 | sed -E 's#^(FDT|fdtfile)=##; s#.*/##')"
...
# Restart ssh service (Fedora: systemd sshd)
mkdir -p -m0755 /var/run/sshd >/dev/null 2>&1 || true
if command -v systemctl >/dev/null 2>&1; then
    (sleep 5 && systemctl restart sshd >/dev/null 2>&1) &
...
# Note: armbian-openvfd is not shipped in the Fedora tree (apt-get based script dropped).
...
if [[ "${openvfd_boxid}" != "0" && "${FDTFILE}" =~ ^meson- ]]; then
    (
        ...
        [[ "${openvfd_enable}" == "yes" ]] && command -v armbian-openvfd >/dev/null 2>&1 && armbian-openvfd "${openvfd_boxid}" >/dev/null 2>&1
        # Some devices require a restart to clear 'BOOT' and related messages
        [[ "${openvfd_restart}" == "yes" ]] && {
            command -v armbian-openvfd >/dev/null 2>&1 && armbian-openvfd "0" >/dev/null 2>&1
            sleep 3
            command -v armbian-openvfd >/dev/null 2>&1 && armbian-openvfd "${openvfd_boxid}" >/dev/null 2>&1
        }
...
# For pveproxy startup service (Fedora: rpm based detection)
if rpm -q pve-manager >/dev/null 2>&1; then
...
# Maximize root partition size (Fedora variant: resize2fs directly, armbian-tf is not shipped)
todo_rootfs_resize="/root/.no_rootfs_resize"
[[ -f "${todo_rootfs_resize}" && "$(cat ${todo_rootfs_resize} 2>/dev/null | xargs)" == "yes" ]] && {
    (sleep 3; rootdev="$(findmnt -n -o SOURCE / 2>/dev/null)"; [[ -n "${rootdev}" ]] && resize2fs "${rootdev}" >/dev/null 2>&1) &
